...
SELECT 
    d.driver_id AS SourceId,
    d.name AS SourceName,
    CONCAT('Driver', d.driver_id, '.png') AS [Source image name],
    t.team_id AS TargetId,
    t.name AS TargetName,
    CONCAT('Team', t.team_id, '.png') AS [Target image name],
    m.season_start,
    m.season_end,
    m.role,
    IIF(m.season_end >= 2024, 'Active', 'Past') AS [Contract status]
FROM member_of m
    INNER JOIN Drivers d ON m.$from_id = d.$node_id
    INNER JOIN Teams t ON m.$to_id = t.$node_id;

SELECT 
    d.driver_id AS SourceId,
    d.name AS SourceName,
    CONCAT('Driver', d.driver_id, '.png') AS [Source image name],
    c.car_id AS TargetId,
    c.model AS TargetName,
    CONCAT('Car', c.car_id, '.png') AS [Target image name],
    dr.year_from,
    dr.year_to,
    dr.wins_on_this_car,
    IIF(dr.wins_on_this_car > 5, 'High success', 'Low success') AS [Success level]
FROM drives dr
    INNER JOIN Drivers d ON dr.$from_id = d.$node_id
    INNER JOIN Cars c ON dr.$to_id = c.$node_id;

SELECT 
    d.driver_id AS SourceId,
    d.name AS SourceName,
    CONCAT('Driver', d.driver_id, '.png') AS [Source image name],
    tr.track_id AS TargetId,
    tr.name AS TargetName,
    CONCAT('Track', tr.track_id, '.png') AS [Target image name],
    w.win_date,
    w.position,
    w.race_name,
    YEAR(w.win_date) AS [Win year]
FROM wins_on w
    INNER JOIN Drivers d ON w.$from_id = d.$node_id
    INNER JOIN Tracks tr ON w.$to_id = tr.$node_id;

SELECT 
    t.team_id AS SourceId,
    t.name AS SourceName,
    CONCAT('Team', t.team_id, '.png') AS [Source image name],
    c.car_id AS TargetId,
    c.model AS TargetName,
    CONCAT('Car', c.car_id, '.png') AS [Target image name],
    u.season,
    u.status,
    IIF(u.status = 'main', 'Primary car', 'Test/Reserve car') AS [Car role]
FROM uses u
    INNER JOIN Teams t ON u.$from_id = t.$node_id
    INNER JOIN Cars c ON u.$to_id = c.$node_id;